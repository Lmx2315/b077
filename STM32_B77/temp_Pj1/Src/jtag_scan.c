...
int jtag_scan(const uint8_t *irlens,uint8_t col)
{
	...
	if (irlens) 
	{
		...
		{
			uint32_t irout;
	//		if(*irlens == 0) //если в массиве стоит "0" в последней ячейки то заканчиваем перебор
			if (col==0)
				break;
			jtag_proc.jtagtap_tdi_tdo_seq((uint8_t*)&irout, 0, ones, *irlens);//шлём еденицы столько раз сколько длинна IR регистра
			if (!(irout & 1)) {
			...
			j += *irlens;
			irlens++; //увеличиваем указатель на ячейку в массиве длин регистров IR
			jtag_dev_count++;
			col--;
		}
	} 
	else //когда переходим в состояние jtagtap_reset в IR регистр записывается адрес IDCODE, для ARM это b1110 !
	{
		Transf("Change state to Shift-IR\n\r");
		jtagtap_shift_ir();
